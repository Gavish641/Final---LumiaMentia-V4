...
# --------------------------------------------Settings----------------
    ...
    def get_last_score_mean(self, last_score_text, mean_title_text, status_rectangle, rectangle_id, text_id):
        ...
        last_decrypted_score = self.encryption.decrypt(eval(last_encrypted_score_details[0]), eval(last_encrypted_score_details[1]), eval(last_encrypted_score_details[2]), eval(last_encrypted_score_details[3]), self.encryption.encryption_keys[self.client.username])
        mean_encrypted_details = json.loads(self.client.messages[1])
        last_decrypted_mean = self.encryption.decrypt(eval(mean_encrypted_details[0]), eval(mean_encrypted_details[1]), eval(mean_encrypted_details[2]), eval(mean_encrypted_details[3]), self.encryption.encryption_keys[self.client.username])
        ...
        if entry_numbers.get() == "":
            ...
            if self.client.messages[2] == "success":
                ...
                decrypted_score = self.encryption.decrypt(
                    eval(encrypted_score_details[0]), 
                    eval(encrypted_score_details[1]), 
                    eval(encrypted_score_details[2]), 
                    eval(encrypted_score_details[3]), 
                    self.encryption.encryption_keys[self.client.username]
                )
