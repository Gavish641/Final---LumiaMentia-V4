...
class Server:
    ...
    def handle_encryption_key(self, msg, sock):
        ...
        print("THE REAL KEY >>> " + str(self.encryption.encryption_keys))
    ...
    def handle_database_queries(self, msg, sock):
        if msg[1] == "check remember me status":
            ...
            self.messages.pop(sock)
            return [json.dumps(self.encryption.encrypt(str(0), self.encryption.encryption_keys[msg[2]])), json.dumps(self.encryption.encrypt(str(0), self.encryption.encryption_keys[msg[2]]))]
